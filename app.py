    ...
    if suffix in (".step", ".stp", ".iges", ".igs"):
        try:
            ...
            if suffix in (".step", ".stp"):
                ...
                try:
                    shape = cq.importers.importIGES(str(input_path))
...
def main():
    ...
    with col_left:
        ...
        if enable_ai_desc:
            st.warning("⚠️ **隐私提示**: 启用 AI 描述功能后，生成的图像将被发送到 DeepSeek API 进行分析。"
                       "请确保您上传的图像不包含敏感或机密信息。图像数据将传输至 DeepSeek 服务器处理。")
