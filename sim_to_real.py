...
def process_directory(input_dir, output_dir, augmentations):
    ...
    if augmentations.get("occlusion", False):
        print("[SimToReal] WARNING: Occlusion was applied but GT annotations were copied as-is.")
        print("[SimToReal]          Mask/pose GT may not match augmented images.")
        print("[SimToReal]          For training-ready data, use --no-occlusion or re-generate COCO annotations.")

    has_coco = (input_path / "coco_annotations.json").exists()
    has_coco_inst = (input_path / "coco_instance_annotations.json").exists()
    if has_coco or has_coco_inst:
        print("[SimToReal] Re-generating COCO annotations from augmented masks...")
        try:
            cmd = [sys.executable, str(Path(__file__).parent / "mask_to_coco.py"),
            ...
            if has_coco_inst:
                cmd_inst = [sys.executable, str(Path(__file__).parent / "mask_to_coco.py"),
                            "--input", str(output_path), "--instance"]
                subprocess.run(cmd_inst, capture_output=True, timeout=600)
            print("[SimToReal] COCO annotations re-generated from augmented data")
        except Exception as e:
            print(f"[SimToReal] COCO re-generation failed: {e}")
            for json_name in ["coco_annotations.json", "coco_instance_annotations.json"]:
                src = input_path / json_name
                if src.exists():
                    import shutil
                    shutil.copy2(str(src), str(output_path / json_name))
