        ...
        while (running) {
            ...
            if (bytesRead > 0) {
                ...
                if (request.find("POST /execute_task") != std::string::npos) {
                    ...
                    const char* envApiKey = std::getenv("HUHB3D_API_KEY");
                    if (envApiKey != nullptr && strlen(envApiKey) > 0) {
                        std::string requiredKey(envApiKey);
                        std::string authHeader = "Authorization: Bearer " + requiredKey;
                        if (request.find(authHeader) == std::string::npos) {
