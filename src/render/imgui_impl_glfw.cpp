...
#ifndef glfwSetWindowFocusCallback
#define glfwSetWindowFocusCallback(w, c) NULL
#define glfwSetCursorEnterCallback(w, c) NULL
#define glfwSetCursorPosCallback(w, c) NULL
#define glfwSetMouseButtonCallback(w, c) NULL
#define glfwSetScrollCallback(w, c) NULL
#define glfwSetKeyCallback(w, c) NULL
#define glfwSetCharCallback(w, c) NULL
#define glfwSetMonitorCallback(c) NULL
#define glfwGetInputMode(w, m) 0
#define glfwSetInputMode(w, m, v)
#define glfwGetClipboardString(w) NULL
#define glfwSetClipboardString(w, s)
#define glfwSetCursorPos(w, x, y)
#define glfwCreateStandardCursor(s) NULL
#define glfwDestroyCursor(c)
#define glfwSetCursor(w, c)
#define glfwGetMonitorWorkarea(m, x, y, w, h)
#define glfwGetMonitorContentScale(m, x, y)
#define glfwGetWindowContentScale(w, x, y)
#define glfwGetJoystickAxes(j, c) NULL
#define glfwGetJoystickButtons(j, c) NULL
#define glfwGetJoystickName(j) NULL
#define glfwGetGamepadState(j, s) 0
#define glfwUpdateGamepadMappings(s) 0
#ifndef GLFW_CURSOR
...
#define GLFW_HAS_WINDOW_TOPMOST 1
#define GLFW_HAS_WINDOW_HOVERED 1
#define GLFW_HAS_WINDOW_ALPHA 1
#define GLFW_HAS_PER_MONITOR_DPI 1
#define GLFW_HAS_VULKAN 1
#define GLFW_HAS_FOCUS_ON_SHOW 1
#define GLFW_HAS_MONITOR_WORK_AREA 1
#define GLFW_HAS_OSX_WINDOW_POS_FIX 1
#define GLFW_HAS_NEW_CURSORS 1
#define GLFW_HAS_GAMEPAD_API 1
...
static void ImGui_ImplGlfw_UpdateGamepads()
{
    ...
    #define MAP_BUTTON(KEY_NO, BUTTON_NO, _UNUSED)          do { if (gamepad.buttons[BUTTON_NO] != 0) io.KeysDown[KEY_NO] = true; } while (0)
    #define MAP_ANALOG(KEY_NO, AXIS_NO, _UNUSED, V0, V1)    do { float v = gamepad.axes[AXIS_NO]; v = (v - V0) / (V1 - V0); io.NavInputs[KEY_NO] = Saturate(v); } while (0)
#else
    ...
    #define MAP_BUTTON(KEY_NO, _UNUSED, BUTTON_NO)          do { if (buttons_count > BUTTON_NO && buttons[BUTTON_NO] == GLFW_PRESS) io.KeysDown[KEY_NO] = true; } while (0)
    #define MAP_ANALOG(KEY_NO, _UNUSED, AXIS_NO, V0, V1)    do { float v = (axes_count > AXIS_NO) ? axes[AXIS_NO] : V0; v = (v - V0) / (V1 - V0); io.NavInputs[KEY_NO] = Saturate(v); } while (0)
#endif
    io.BackendFlags |= ImGuiBackendFlags_HasGamepad;
