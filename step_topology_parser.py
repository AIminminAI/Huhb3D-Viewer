...
def parse_step_topology(step_path, output_dir, linear_deflection=0.1, angular_deflection=0.5):
    ...
    while explorer.More():
        ...
        # Check for degenerate face (extremely small area relative to bounding box)
        max_dim = max(xmax - xmin, ymax - ymin, zmax - zmin)
        relative_area = face_area / (max_dim * max_dim) if max_dim > 0 else 1.0
        if face_area < 1e-6 or relative_area < 0.001:
            geom_type_str = "DEGENERATE"
            category_id = 7  # Degenerate
            category_name = CATEGORY_NAMES[7]
            extra_info = {"face_area": face_area, "relative_area": relative_area}
        ...
        face_triangulation = BRep_Tool.Triangulation_s(face, TopLoc_Location())
        tri_count_for_face = 0

        if face_triangulation is not None:
            n_nodes = face_triangulation.NbNodes()
            n_triangles = face_triangulation.NbTriangles()

            trsf = face.Location().Transformation()

            for t_idx in range(1, n_triangles + 1):
                tri = face_triangulation.Triangle(t_idx)
                n1_idx, n2_idx, n3_idx = tri.Get()

                v1 = face_triangulation.Node(n1_idx)
                v2 = face_triangulation.Node(n2_idx)
                v3 = face_triangulation.Node(n3_idx)
